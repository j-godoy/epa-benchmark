...
public class ListItr implements ListIterator<Object> {
	...
	private boolean isNextEnabled() {
		return this.arrayList.getModCount() == expectedModCount && cursor < this.arrayList.size() && cursor < arrayList.getElementData().length;
	}

	@EpaActionPrecondition(name = "remove")
	private boolean isRemoveEnabled() {
		return lastRet >= 0 && this.arrayList.getModCount() == expectedModCount;
	}

	@EpaActionPrecondition(name = "previous")
	private boolean isPreviousEnabled() {
		return this.arrayList.getModCount() == expectedModCount && cursor - 1 < arrayList.getElementData().length && cursor - 1 >= 0;
	}
