...
class StrategyEvosuiteGeneration(Enum):
    EVOSUITE = '-generateSuite'
    RANDOM = '-generateRandom'
    RANDOM_FIXED = '-generateNumRandom'
    REGRESSION = '-regressionSuite'
    REGRESSIONTESTS = '-regressionTests'
    MOSUITE = '-generateMOSuite'
...
def run_randoop(projectCP, class_name, randoop_jar_path, testdir, search_budget):
    ...
    command = 'java -classpath {}{}{} randoop.main.Main gentests --testclass={} --time-limit={} --usethreads=true --junit-package-name={} --npe-on-non-null-input=expected --junit-output-dir={} > {} 2> {}'.format(projectCP, sep, randoop_jar_path, class_name, search_budget, packages_dir.replace(os.path.sep, ".")[:-1], testdir, out_file, err_file)
    utils.print_command(command)
    try:
        subprocess.check_output(command, shell=True)
    except Exception as e:
