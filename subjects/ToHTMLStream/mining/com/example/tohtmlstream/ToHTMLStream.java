...
public class ToHTMLStream extends ToStream {
	 ...
	 */
	private final MockCharInfo m_htmlcharInfo =
	// new CharInfo(CharInfo.HTML_ENTITIES_RESOURCE);
	MockCharInfo.getCharInfo(MockCharInfo.HTML_ENTITIES_RESOURCE, Method.HTML);

	/**
	 * A digital search trie for fast, case insensitive lookup of ElemDesc
	 * objects.
	 */
	private static final Trie m_elementFlags = new Trie();
...
//	}
	 ...
	 */
	@EpaAction(name = "setOutputFormat(Properties)")
	public void setOutputFormat(Properties format) {
		/*
		 * If "format" does not contain the property S_USE_URL_ESCAPING, then
		 * don't set this value at all, just leave as-is rather than explicitly
		 * setting it.
		 */
		String value;
		value = format.getProperty(OutputPropertiesFactory.S_USE_URL_ESCAPING);
		if (value != null) {
			m_specialEscapeURLs = OutputPropertyUtils.getBooleanProperty(OutputPropertiesFactory.S_USE_URL_ESCAPING,
					format);
		}

		/*
		 * If "format" does not contain the property S_OMIT_META_TAG, then don't
		 * set this value at all, just leave as-is rather than explicitly
		 * setting it.
		 */
...
//	}
	 ...
	 */
	@EpaAction(name = "startDocumentInternal()")
	public void startDocumentInternal() throws MockSAXException, MockIOException {
	...
	/**
	 * This method should only get called once. If a DOCTYPE declaration needs
	 * to get written out, it will be written out. If it doesn't need to be
	 * written out, then the call to this method has no effect.
	 */
	private void outputDocTypeDecl(String name) throws MockSAXException {
	 ...
	 * @throws MockIOException 
	 */
	@EpaAction(name = "endDocument()")
	public final void endDocument() throws MockSAXException, MockIOException {
	 ...
	 */
	@EpaAction(name = "startElement(String,String,String,MockAttributes)")
	public void startElement(String namespaceURI, String localName, String name, MockAttributes atts)
			throws MockSAXException {
...
//				elemContext.m_isRaw = (elemFlags & ElemDesc.RAW) != 0;
	 ...
	 *             Any SAX exception, possibly wrapping another exception.
	 */
	@EpaAction(name = "endElement(String,String,String)")
...
//	}
	 ...
	 * @throws MockIOException 
	 */
	@EpaAction(name = "characters(char[],int,int)")
	public final void characters(char chars[], int start, int length) throws MockSAXException, MockIOException {
		...
		if (m_elemContext.m_isRaw) {
			try {
				...
				throw new MockSAXException("", ioe);
			}
		} else {
...
//	 * @throws MockSAXException
//	 */
//	public final void cdata(char ch[], int start, int length) throws MockSAXException {
//
...
//			} catch (MockIOException ioe) {
//				throw new MockSAXException(Utils.messages.createMessage(MsgKey.ER_OIERROR, null), ioe);
//				// "IO error", ioe);
//			}
//		} else {
...
//	 * @throws MockSAXException
//	 */
//	public void processingInstruction(String target, String data) throws MockSAXException {
//
...
//
//				/*
//				 * Perhaps processing instructions can be written out in HTML
//				 * before the DOCTYPE, in which case this could be emitted with
//				 * the startElement call, that knows the name of the document
//				 * element doing it right.
//				 */
//				if (true == m_needToOutputDocTypeDecl)
//					outputDocTypeDecl("html"); // best guess for the upcoming
//												// element
//
//				if (shouldIndent())
//					indent();
...
//	 * @throws MockSAXException
//	 */
//	public final void entityReference(String name) throws MockSAXException {
//		try {
//
//			final java.io.Writer writer = m_writer;
//			writer.write('&');
...
//	}
	 ...
	 */
	@EpaAction(name = "endElement(String)")
	public final void endElement(String elemName) throws MockSAXException {
...
//
//			/*
//			 * At this point we have the prefix mappings now, so lets determine
//			 * if the current element is specified in the cdata-
...
//
//		/*
//		 * Perhaps comments can be written out in HTML before the DOCTYPE. In
//		 * this case we might delete this call to writeOutDOCTYPE, and it would
//		 * be handled within the startElement() call.
//		 */
//		if (m_needToOutputDocTypeDecl)
...
//	}
	...
	static class Trie {
		 ...
		 * are put in it.
		 * 
		 * This class is a copy of the one in org.apache.xml.utils. It exists to
...
//		}
	...
	@EpaActionPrecondition(name = "setOutputFormat(Properties)")
	private boolean isSetOutputFormatEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "startDocumentInternal()")
	private boolean isStartDocumentInternalEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "endDocument()")
	private boolean isEndDocumentEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "startElement(String,String,String,MockAttributes)")
	private boolean isStartElementEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "endElement(String)")
	private boolean isEndElementstringEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "endElement(String,String,String)")
	private boolean isEndElementStringStringStringEnabled() {
		return true;
	}
	
	@EpaActionPrecondition(name = "characters(char[],int,int)")
	private boolean isCharactersEnabled() {
		return true;
	}
