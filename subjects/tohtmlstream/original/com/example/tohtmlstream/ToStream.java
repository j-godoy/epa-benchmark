 ...
 * @xsl.usage internal
 */
abstract public class ToStream extends SerializerBase implements SerializerConstants
{
     ...
     */
    protected MockCharInfo m_charInfo;
     ...
     *
     * @throws MockSAXException 
     */
    protected void closeCDATA() throws MockSAXException
    {
        try {
...
//     * @throws MockIOException An I/O exception occured while serializing
//     */
//    public void serialize(Node node) throws MockIOException
//    {
//
//        try
//        {
...
//        }
//        catch (MockSAXException se)
//        {
//            throw new MockWrappedRuntimeException(se);
//        }
//    }
     ...
     * @throws MockSAXException
     */
    protected final void flushWriter() throws MockSAXException
    {
        final MockWriter writer = m_writer;
        if (null != writer)
        {
...
//				e.printStackTrace();
    ...
    MockOutputStream m_outputStream;
    /**
...
//    }
     ...
     * @throws MockSAXException
     */
//    void outputEntityDecl(String name, String value) throws MockIOException
//    {
//        final MockWriter writer = m_writer;
//        writer.write("<!ENTITY ");
//        writer.write(name);
...
//    }
     ...
     * @throws MockSAXException
     */
    protected final void outputLineSep() throws MockIOException
    {
	...
	public void setOutputFormat(Properties format)
    {
        ...
        if (null != entitiesFileName)
        {
            ...
            m_charInfo = MockCharInfo.getCharInfo(entitiesFileName, method);
        }
     ...
     */
//    public void setWriter(MockWriter writer)
//    {        
//        setWriterInternal(writer, true);
//    }
    
    private boolean m_writer_set_by_user;
    private void setWriterInternal(MockWriter writer, boolean setByUser) {
...
//            }
     ...
     */
    public void setOutputStream(MockOutputStream output)
    {
        setOutputStreamInternal(output, true);
    }
    
    private void setOutputStreamInternal(MockOutputStream output, boolean setByUser)
    {
                ...
                || "ASCII".equals(encoding))
        {
            ...
            MockWriter osw = null;
                ...
                catch (MockUnsupportedEncodingException uee)
                {
                    osw = null;
                }
            ...
            if (osw == null) {
                ...
                try {
                    osw = Encodings.getWriter(output, encoding);
                } catch (MockUnsupportedEncodingException e) {
...
//                    e.printStackTrace();
            ...
            MockWriter osw = new MockOutputStreamWriter(output);
            setWriterInternal(osw,false);
        }
    }
     ...
     *
     * @throws MockSAXException if an error occurs during writing.
     */
    protected void indent(int depth) throws MockIOException
    {
     ...
     * @throws MockIOException
     */
    protected void indent() throws MockIOException
    {
     ...
     *
     * @throws MockSAXException if an error occurs when writing.
     */
    private void printSpace(int n) throws MockIOException
    {
        final MockWriter writer = m_writer;
        for (int i = 0; i < n; i++)
        {
...
//    }
    ...
    protected boolean escapingNotNeeded(char ch)
    {
        ...
        if (ch < 127)
        {
            ...
            if (ch >= MockCharInfo.S_SPACE || (MockCharInfo.S_LINEFEED == ch || 
                    MockCharInfo.S_CARRIAGERETURN == ch || MockCharInfo.S_HORIZONAL_TAB == ch))
                ret= true;
            else
                ret = false;
        }
     ...
     * 
     * @throws MockIOException
     * @throws MockSAXException if invalid UTF-16 surrogate detected.
     */
        ...
        throws MockIOException
    {
        ...
        if (!escLF && MockCharInfo.S_LINEFEED == ch)
        {
            writer.write(m_lineSep, 0, m_lineSepLen);
        }
        else
    ...
    void writeNormalizedChars(
        ...
        throws MockIOException, MockSAXException
    {
        final MockWriter writer = m_writer;
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (MockCharInfo.S_LINEFEED == c && useSystemLineSeparator)
            {
                writer.write(m_lineSep, 0, m_lineSepLen);
            }
     ...
     * @throws MockSAXException
     */
    public void endNonEscaping() throws MockSAXException
    {
     ...
     * @throws MockSAXException
     */
    public void startNonEscaping() throws MockSAXException
    {
     ...
     * @param start The start position in the array.
     * @param length The number of characters to read from the array.
     * @throws MockSAXException Any SAX exception, possibly
    ...
    protected void charactersRaw(char ch[], int start, int length)
        throws MockSAXException, MockIOException
    {
        ...
        try
        {
            ...
            while (i < end && isAllWhitespace) {
                ...
                if (m_charInfo.shouldMapTextChar(ch1)) {
                    ...
                    case MockCharInfo.S_SPACE:
                        // Just accumulate the clean whitespace
                        i++;
                        break;
                    case MockCharInfo.S_LINEFEED:
                        lastDirtyCharProcessed = processLineFeed(chars, i,
                                lastDirtyCharProcessed, writer);
                        i++;
                        break;
                    case MockCharInfo.S_CARRIAGERETURN:
                        writeOutCleanChars(chars, i, lastDirtyCharProcessed);
                        writer.write("&#13;");
                        lastDirtyCharProcessed = i;
                        i++;
                        break;
                    case MockCharInfo.S_HORIZONAL_TAB:
                        // Just accumulate the clean whitespace
                        i++;
                        break;
                    default:
            ...
            for (; i < end; i++)
            {
                ...
                if (m_charInfo.shouldMapTextChar(ch)) {
                    ...
                    else if (ch == MockCharInfo.S_LINE_SEPARATOR) {
                        // LINE SEPARATOR
                        writeOutCleanChars(chars, i, lastDirtyCharProcessed);
                        writer.write("&#8232;");
                        lastDirtyCharProcessed = i;
	...
	private int processLineFeed(final char[] chars, int i, int lastProcessed, final MockWriter writer) throws MockIOException {
		if (!m_lineSepUse 
		|| (m_lineSepLen ==1 && m_lineSep[0] == MockCharInfo.S_LINEFEED)){
    ...
    private void writeOutCleanChars(final char[] chars, int i, int lastProcessed) throws MockIOException {
        int startClean;
...
//        char ch,
//        int lastDirty,
//        boolean fromTextNode) throws MockIOException
//    {
...
//        if (MockCharInfo.S_LINEFEED == ch && fromTextNode)
//        {
//            m_writer.write(m_lineSep, 0, m_lineSepLen);
//        }
//        else
...
//    }
     ...
     * @throws MockIOException 
     */
    public void characters(String s) throws MockSAXException, MockIOException
    {
    ...
    public void startElement(String elementName) throws MockSAXException, MockIOException
    {
     ...
     * @throws java.io.IOException
     */
    void outputDocTypeDecl(String name, boolean closeDecl) throws MockSAXException, MockIOException
    {
        if (m_cdataTagOpen)
            closeCDATA();
        final MockWriter writer = m_writer;
     ...
     * @throws MockSAXException
     */
    public void processAttributes(MockWriter writer, int nAttrs) throws MockIOException, MockSAXException
    {
     ...
     * @throws MockIOException 
     */
    public void endElement(String name) throws MockSAXException, MockIOException
    {
     ...
     * 
     * @throws MockSAXException The client may throw
     ...
     * @throws MockSAXException The application may raise an exception.
     * @throws MockIOException 
     */
     ...
     *  @see  #startCDATA
     */
    public void endCDATA() throws MockSAXException
    {
        if (m_cdataTagOpen)
            closeCDATA();
     ...
     * @see #startDTD
     */
    public void endDTD() throws MockSAXException
    {
        try
        {
            if (m_needToOutputDocTypeDecl)
...
//			e.printStackTrace();
     ...
     *            an exception during processing.
     */
    public void endPrefixMapping(String prefix) throws MockSAXException
    { // do nothing
    }
     ...
     * "[dtd]".
     * @throws MockSAXException Any SAX exception, possibly wrapping
     * another exception.
     */
    public void skippedEntity(String name) throws MockSAXException
    { // TODO: Should handle
    }
     ...
     * @see #endCDATA
     */
    public void startCDATA() throws MockSAXException
    {
     ...
     */
    public void startEntity(String name) throws MockSAXException, MockIOException
    {
        if (name.equals("[dtd]"))
     ...
     * @throws MockSAXException
     */
    protected void closeStartTag() throws MockSAXException
    {
     ...
     *        external DTD subset, or null if none was declared.
     * @throws MockSAXException The application may raise an
     *            exception.
     * @see #endDTD
     * @see #startEntity
     */
...
//    }
     ...
     * @throws MockIOException 
     */
    public void flushPending() throws MockSAXException, MockIOException
    {
            if (m_needToCallStartDocument)
            {
                startDocumentInternal();
...
//    }
     ...
     * the same code.
     */
    private static class WritertoStringBuffer extends MockWriter
    {
        final private StringBuffer m_stringbuf;
        /**
         * @see MockWriter#write(char[], int, int)
         */
        ...
        public void write(char[] arg0, int arg1, int arg2) throws MockIOException
        {
            m_stringbuf.append(arg0, arg1, arg2);
        }
        /**
         * @see MockWriter#flush()
         */
        public void flush() throws MockIOException
        {
        }
        /**
         * @see MockWriter#close()
         */
...
//     }
     ...
     */
    public void notationDecl(String name, String pubID, String sysID) throws MockSAXException {
...
//            e.printStackTrace();
     ...
     */
    public void unparsedEntityDecl(String name, String pubID, String sysID, String notationName) throws MockSAXException {
        // TODO Auto-generated method stub
        try {
            DTDprolog();       
...
//            e.printStackTrace();
     ...
     * @throws MockIOException
     */
    private void DTDprolog() throws MockSAXException, MockIOException {
        final MockWriter writer = m_writer;
        if (m_needToOutputDocTypeDecl)
        {
            outputDocTypeDecl(m_elemContext.m_elementName, false);
