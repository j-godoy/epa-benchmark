...
printHeader <- function()
{
	...
	cat("SUBJECT", "BUDGET", "CRITERION", "REP", "LINE", "BRNCH", "EPA%", "EXCEP%", "ADJAC%", "PIMUT", sep=", ")
	cat("\n")
}
...
printPitMutationScoreMedian <- function() {
	for(subj in subjects) {
		...
		for (budget in budgets) {
			...
			cat(name_subj, budget, criterion, repeticiones, line_avg, brnch_avg, paste(epacov_avg, epa, sep=""), paste(excepcov_avg, excep, sep=""), paste(adjaccov_avg, adjac, sep=""), pit_avg, sep=", ")
			cat("\n")
			}
		}
	}
