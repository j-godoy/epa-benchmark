	...
	for(subj in subjects)
	{
		for (budget in budgets)
		{
			if (budget=='600')
			{
				...
				boxplot(default_pit, line_branch_exception_epatransition_pit, line_branch_exception_epatransition_epaexception_pit, line_branch_exception_edges_pit, names=c("Base","Base+Epa","Base+Epax","Base+Epaxp"),main=c(name_subj),ylab="PIT Score",xlab=budget_txt,cex.lab=1.5, cex.axis=1.2)
				...
				pdf(outputname)
				boxplot(default_pit, epatransition_pit, epatransition_epaexception_pit, edges_pit, names=c("Base","Epa","Epax","Epaxp"),main=c(name_subj),ylab="PIT Score",xlab=budget_txt,cex.lab=1.5, cex.axis=1.6)
			}
		}
	}
}
