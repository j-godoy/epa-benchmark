...
decimals_size_pvalue = 5
decimals_size_a12 = 2
digits_size_to_percentage = 1

printHeader <- function()
{
	cat("Subject","Evosuite vs. EPA","Evosuite vs. EPA","Evosuite vs. EPA","Evosuite vs. EPA","Evosuite vs. EPAEX","Evosuite vs. EPAEX","Evosuite vs. EPAEX","Evosuite vs. EPAEX","Evosuite vs. EPAXP","Evosuite vs. EPAXP","Evosuite vs. EPAXP","Evosuite vs. EPAXP",sep=", ")
	cat("\n")
	cat("Subject","AVG Tx","AVG Tx","A12", "p-value","AVG EXTx","AVG EXTx","A12", "p-value","AVG TxPairs","AVG TxPairs","A12", "p-value",sep=", ")
	cat("\n")
	cat("Subject","Evosuite","EPA","A12", "p-value","Evosuite","EPAEX","A12", "p-value","Evosuite","EPAXP","A12", "p-value",sep=", ")
	cat("\n")
	cat("EOH\n")
}
	...
	bug_type_var  = "all"
		for (budget in budgets) {
			for(subj in subjects) {
				...
				default_epax = default_rows$EPAEXCEPMINING
				default_epaxp = default_rows$EDGESMINING
				
				# EPATRANSITIONMINING
				epatransition_rows  = subset(stats,SUBJ==subj & TOOL=='epatransitionmining' & BUD==budget & BUG_TYPE==bug_type_var)
				epatransition_epa_tx = roundDecimals(round(mean(epatransition_rows$EPAMINING), digits=digits_size_to_percentage))
				epatransition_epa = epatransition_rows$EPAMINING
				epatransition_a12 = round(measureA(default_epa_rows, epatransition_epa),digits=decimals_size_a12)
				epatransition_p_value = pValueRefactor(wilcox.test(default_epa_rows, epatransition_epa)$p.value)
				
				# EPAEXCEPTIONMINING
				epa_epax_rows  = subset(stats,SUBJ==subj & TOOL=='epaexceptionmining' & BUD==budget & BUG_TYPE==bug_type_var)
				epa_epax_tx = roundDecimals(round(mean(epa_epax_rows$EPAEXCEPMINING), digits=digits_size_to_percentage))
				epa_epax = epa_epax_rows$EPAEXCEPMINING
				epa_epax_a12 = round(measureA(default_epax, epa_epax),digits=decimals_size_a12)
				epa_epax_p_value = pValueRefactor(wilcox.test(default_epax, epa_epax)$p.value)
				
				# EPAADJACENTEDGESMINING
				edges_rows  = subset(stats,SUBJ==subj & TOOL=='epaadjacentedgesmining' & BUD==budget & BUG_TYPE==bug_type_var)
				edges_epaxp_tx = roundDecimals(round(mean(edges_rows$EDGESMINING), digits=digits_size_to_percentage))
				edges_epaxp = edges_rows$EDGESMINING
				edges_a12 = round(measureA(default_epaxp, edges_epaxp),digits=decimals_size_a12)
				edges_p_value = pValueRefactor(wilcox.test(default_epaxp, edges_epaxp)$p.value)
