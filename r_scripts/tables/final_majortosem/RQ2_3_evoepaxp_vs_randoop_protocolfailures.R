...
printHeader <- function()
{
	...
	cat("\n")
	cat("Subject","Randoop","#pf","A12","p-value","#pf Diff",sep=", ")
	cat("\n")
	cat("EOH\n")
}
...
RQ3_2 <- function()
{
	...
	for(subj in subjects)
	{
		...
		for (budget in budgets)
		{
			...
			evoepaxp_protocolmean = round(mean(evoepaxp_protocol), digits=digits_size_to_percentage)
			
			# RANDOOP
			randoop_rows  = subset(stats,SUBJ==subj & TOOL=='randoop' & BUD==budget & BUG_TYPE==bug_type_errprot)
			randoop_protocol = randoop_rows$KILLED_PIMUT
			randoop_protocolmean = round(mean(randoop_protocol), digits=digits_size_to_percentage)
			...
			pfdiff =  round((evoepaxp_protocolmean / randoop_protocolmean - 1) * 100, digits=decimals_size_pvalue)
			if(pfdiff > 0)
				pfdiff = paste("+",pfdiff,sep="")
			
			
			cat(", ", roundDecimals(randoop_protocolmean), ", ", roundDecimals(evoepaxp_protocolmean), ", ", randoop_a12_vsevoepaxp,", ",randoop_p_value_vsevoepaxp, ", ", pfdiff, "%", sep="")
			cat("\n")
		}
	}
}
