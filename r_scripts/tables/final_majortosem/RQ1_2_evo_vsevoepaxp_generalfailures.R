...
{
	cat("Subject"," mu Evosuite","Evosuite+EPAXP","Evosuite+EPAXP","Evosuite+EPAXP", sep=", ")
	cat("\n")
	cat("Subject","mu Evosuite","mu","A12","p-value",sep=", ")
	cat("\n")
	cat("EOH\n")
}
...
RQ2_1 <- function()
{
	...
	for (budget in budgets) {
		for(subj in subjects) {
			...
			cat(", ", default_pitmean, "%, ", edges_pitmean, "%, ", edges_a12, ", ", edges_p_value, sep="")
			cat("\n")
		}
	}
}
