	...
	bug_type_all  = "all"
		for (budget in budgets) {
			for(subj in subjects) {
				...
				epatransition_pitmean = round(mean(epatransition_pit)*100, digits=digits_size_to_percentage)
				
				# EPAEXCEPTIONMINING
				epa_epax_rows  = subset(stats,SUBJ==subj & TOOL=='epaexceptionmining' & BUD==budget & BUG_TYPE==bug_type_all)
				epa_epax_pit = epa_epax_rows$PIMUT
				epa_epax_pitmean = round(mean(epa_epax_pit)*100, digits=digits_size_to_percentage)
				...
				lbeepatransition_rows  = subset(stats,SUBJ==subj & TOOL=='line_branch_exception_epatransitionmining' & BUD==budget & BUG_TYPE==bug_type_all)
				lbeepatransition_pit = lbeepatransition_rows$PIMUT
				lbeepatransition_pitmean = round(mean(lbeepatransition_pit)*100, digits=digits_size_to_percentage)
				
				# LINE:BRANCH:EXCEPTION:EPAEXCEPTIONMINING
				lbeepa_epax_rows  = subset(stats,SUBJ==subj & TOOL=='line_branch_exception_epaexceptionmining' & BUD==budget & BUG_TYPE==bug_type_all)
				lbeepa_epax_pit = lbeepa_epax_rows$PIMUT
				lbeepa_epax_pitmean = round(mean(lbeepa_epax_pit)*100, digits=digits_size_to_percentage)
				
				# LINE:BRANCH:EXCEPTION:EPAADJACENTEDGESMINING
				lbeedges_rows  = subset(stats,SUBJ==subj & TOOL=='line_branch_exception_epaadjacentedgesmining' & BUD==budget & BUG_TYPE==bug_type_all)
				lbeedges_pit = lbeedges_rows$PIMUT
				lbeedges_pitmean = round(mean(lbeedges_pit)*100, digits=digits_size_to_percentage)
				
				###Ranking
				pit_values <- c((default_pitmean), (epatransition_pitmean), epa_epax_pitmean, edges_pitmean, lbeepatransition_pitmean, lbeepa_epax_pitmean, lbeedges_pitmean)
				sorted_pit_values <- sort(pit_values, method = "quick", decreasing=TRUE) # Orden inverso
