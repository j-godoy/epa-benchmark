...
RQ2_1 <- function()
{
	for(subj in subjects)
	{
		...
		for (budget in budgets)
		{
			...
			epatransition_a12 = round(measureA(default_rows$PIMUT, epatransition_rows$PIMUT),digits=decimals_size_a12)
			epatransition_p_value = pValueRefactor(wilcox.test(default_rows$PIMUT, epatransition_rows$PIMUT)$p.value)
			...
			edges_a12 = round(measureA(default_rows$PIMUT, edges_rows$PIMUT),digits=decimals_size_a12)
			edges_p_value = pValueRefactor(wilcox.test(default_rows$PIMUT, edges_rows$PIMUT)$p.value)
