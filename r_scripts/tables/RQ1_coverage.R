...
RQ1 <- function()
{
	for(subj in subjects)
	{
		...
		for (budget in budgets)
		{
			...
			default_epaex = round(mean(default_rows$EXCEPVALCOV)*100, digits=2)
			...
			epatransition_epaexception_epaex = round(mean(epatransition_epaexception_rows$EXCEPVALCOV)*100, digits=2)
